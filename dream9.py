...
def a_star(start, goal, grid):
    ...
    while oheap:
        ...
        for i, j in neighbors:
            ...
            if tentative g_score < gscore.get(neighbor, 0) or neighbor not in [i[1] for i in oheap]:
...
def decide_next_move(mood, position, map, front_dist, rear_dist, tof_dist, prox_dist, gyro_data):
    # Adjusted proximity threshold to 10 cm (~4 inches)
...
async def fetch_with_retries(session, url, max_retries=10, timeout=10):
