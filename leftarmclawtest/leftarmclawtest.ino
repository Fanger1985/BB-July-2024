...
bool autoMode = false;
bool clawsOpen = false; // Claws state
...
void setup() {
    ...
    server.on("/toggleClaws", handleToggleClaws);
...
void handleRoot() {
    ...
    <button class='button' onclick=\"sendRequest('/toggleClaws')\">Toggle Claws</button>\
...
// Handle URL "/toggleClaws"
void handleToggleClaws() {
    clawsOpen = !clawsOpen;
    if (clawsOpen) {
        setServoAngle(0, 180); // Open claws
    } else {
        setServoAngle(0, 0); // Close claws
    }
    server.send(200, "text/plain", clawsOpen ? "Claws opened" : "Claws closed");
}
