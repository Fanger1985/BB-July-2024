...
# Initialize Q-table
Q_TABLE = np.zeros((10, 10, 8))  # 10x10 grid with 8 possible actions (including the new tasks)
ACTIONS = ['/forward', '/backward', '/spinLeft', '/spinRight', '/stop', '/task_a', '/task_b', '/task_c']
...
def decide_next_move(mood, position, map, front_dist, rear_dist, tof_dist, gyro_data):
    if mood == "happy" and front_dist > 500 and tof_dist > 500:
        return '/forward'
    elif mood == "curious" and front_dist > 250:
        return '/spinRight'
...
def get_reward(front_distance, rear_distance, tof_distance):
    if front_distance < 200 or tof_distance < 200:
        return -100
    if front_distance > 500 or tof_distance > 500:
        return 50
    return -1
...
async def fetch_with_retries(session, url, max_retries=10, timeout=15):
...
async def get_sensor_data(session):
    while True:
        try:
            ...
            logging.debug("Fetching gyro data...")
            ...
            if sensor_data and gyro_data:
                ...
                logging.error("Failed to fetch sensor or gyro data.")
...
async def make_decision_based_on_sensors(sensor_data, gyro_data):
    try:
        ...
        tof_distance = tof_sensor.distance  # Real-time ToF distance update
        ...
        state = get_state()
        action = decide_next_move(mood, state, MAP_GRID, front_distance, rear_distance, tof_distance, gyro_data)

        set_ears_based_on_action(action)

        await send_command(action)
        next_state = get_state()
        reward = get_reward(front_distance, rear_distance, tof_distance)
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{BASE_URL}{endpoint}", timeout=15) as response:
                    if response.status == 200:
                        logging.debug(f'Command {BASE_URL}{endpoint} executed successfully!')
                        return
                    else:
                        logging.error(f'Failed to execute {BASE_URL}{endpoint}: Status {response.status}')
        except Exception as e:
            logging.error(f'Error sending command {BASE_URL}{endpoint} on attempt {attempt + 1}: {e}')
            await asyncio.sleep(3)  # Wait before retrying

    logging.error(f'Failed to send command {BASE_URL}{endpoint} after {max_retries} attempts')
...
def set_ears_based_on_action(action):
    ...
    elif action in ["/task_a", "/task_b", "/task_c"]:
        move_ears(0, 0)  # Ears in resting position
